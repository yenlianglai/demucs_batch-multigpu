...
from .apply import BagOfModels as BOM
    ...
    def max_allowed_segment(self) -> float:
        ...
        for model in self.models:
            if isinstance(model, HTDemucs):
                max_allowed_segment = min(max_allowed_segment, float(model.module.segment))
...
def apply_model(model: tp.Union[BOM, Model],
    ...
    if isinstance(model.module, BOM):
        # Special treatment for bag of model.
        # We explicitely apply multiple times `apply_model` so that the random shifts
        # are different for each model.
        estimates: tp.Union[float, th.Tensor] = 0.
        totals = [0.] * len(model.module.sources)
        for sub_model, model_weights in zip(model.module.models, model.module.weights):
            original_model_device = next(iter(sub_model.parameters())).device
            sub_model = th.nn.DataParallel(sub_model)
            sub_model.to(device)
    ...
    if shifts:
        kwargs['shifts'] = 0
        max_shift = int(0.5 * model.module.samplerate)
        ...
        kwargs['split'] = False
        out = th.zeros(batch, len(model.module.sources), channels, length, device=mix.device)
        sum_weight = th.zeros(length, device=mix.device)
        if segment is None:
            segment = model.module.segment
        assert segment is not None and segment > 0.
        segment_length: int = int(model.module.samplerate * segment)
        stride = int((1 - overlap) * segment_length)
        offsets = range(0, length, stride)
        scale = float(format(stride / model.module.samplerate, ".2f"))
        ...
        if isinstance(model, HTDemucs) and segment is not None:
            valid_length = int(segment * model.module.samplerate)
        elif hasattr(model, 'valid_length'):
            valid_length = model.module.valid_length(length)  # type: ignore
        else:
            valid_length = length
        mix = tensor_chunk(mix)
        assert isinstance(mix, TensorChunk)
